...
Bhv_SetPlay::execute( PlayerAgent * agent )
{
    ...
    if(wm.self().goalie())
    {
        ...
        {
            Body_GoToPoint(Vector2D(-52, 0), 0, 100).execute(agent);
            return true;
        }
    }
...
Bhv_SetPlay::doMove( PlayerAgent * agent )
{
    ...
    if(wm.self().goalie())
    {
        target_point = Bhv_GoalieBasicMove().getTargetPoint(agent);
    }
    if(!wm.gameMode().isOurSetPlay(wm.ourSide()))
    {
        if(wm.ball().pos().x > -50)
            target_point = Vector2D(Bhv_BasicMove().DeffenseFormation(agent, wm.self().unum()).x - 2, Bhv_BasicMove().DeffenseFormation(agent, wm.self().unum()).y);
        if(wm.ball().pos().x < -51)
            target_point = Bhv_BasicMove().DeffenseFormation(agent, wm.self().unum());
    }
    else
    {
        target_point = Strategy().Get_Formation(agent, wm.self().unum(), Strategy().Get_RecommendedStragetyType(agent));
    }
    if(wm.gameMode().isOurSetPlay(wm.ourSide()) && wm.gameMode().type() == GameMode::CornerKick_)
    {
        target_point = Bhv_BasicMove().CornerForamtion(agent);
    }
    if(wm.gameMode().type() == GameMode::GoalKick_ && wm.gameMode().isOurSetPlay(wm.ourSide()))
    {
        target_point.x = target_point.x + 10;
    }
...
Bhv_SetPlay::doKick( PlayerAgent * agent )
{
    ...
    // pass
    //
    if(wm.gameMode().type() == rcsc::GameMode::GoalKick_ && wm.gameMode().side() == wm.ourSide()){
        if(Bhv_BasicOffensiveKick().pass_to_forward(agent)){
        return;
    }
    }
...
#ifdef DEBUG_PRINT
    dlog.addText( Logger::TEAM,
                  _FILE_": (get_avoid_circle_point) first_target=(%.2f %.2f)",
                  target_point.x, target_point.y );
...
#ifdef DEBUG_PRINT
    dlog.addText( Logger::TEAM,
                  _FILE_": (get_avoid_circle_point) ball_target_angle=%.1f angle_step=%d",
 ...
 */
bool
Bhv_SetPlay::is_kicker(PlayerAgent * agent )
{
    const WorldModel & wm = agent->world();
    static bool is_Me = false;
    static int Cy = wm.time().cycle();
    if(wm.time().cycle() - Cy >= 100)
        Cy = wm.time().cycle();
    if(Cy < 100 && is_Me)
        return true;
    if(wm.self().pos().dist(wm.ball().pos()) < wm.getDistTeammateNearestTo(wm.ball().pos(), 5))
    {
        is_Me = true;
        Cy = wm.time().cycle();
        return true;
