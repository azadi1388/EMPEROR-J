...
Bhv_GoalieBasicMove::execute( PlayerAgent * agent )
{
    ...
    if(wm.ball().pos().x > -20)
    {
        move_point.x = move_point.x + wm.ball().pos().dist(Vector2D(-52,0)) / 4;
    }
    ...
    if ( Bhv_BasicTackle( 0.01, 10 ).execute( agent ) )
    {
        return true;
    }}
    Sector2D sector = Sector2D(wm.ball().pos(), 1, wm.ball().pos().dist(Vector2D(-52,0)), (Vector2D(-52,0) - wm.ball().pos()).th() + 15, (Vector2D(-52,0) - wm.ball().pos()).th() - 15);
    if(!wm.existTeammateIn(sector,5,false) && wm.lastKickerSide() != wm.ourSide()){
        Body_Intercept().execute(agent);
    return true;}
